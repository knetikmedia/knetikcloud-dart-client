...
> Page«AggregateInvoiceReportResource» getDailyInvoicesUsingGET(currencyCode, granularity, filterPaymentStatus, filterFulfillmentStatus, startDate, endDate)
...
try { 
    var result = api_instance.getDailyInvoicesUsingGET(currencyCode, granularity, filterPaymentStatus, filterFulfillmentStatus, startDate, endDate);
    print(result);
} catch (e) {
    print("Exception when calling ReportingOrdersApi->getDailyInvoicesUsingGET: $e\n");
}
```
...
[**Page«AggregateInvoiceReportResource»**](Page«AggregateInvoiceReportResource».md)
