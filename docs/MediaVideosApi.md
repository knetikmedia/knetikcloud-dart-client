...
try { 
    api_instance.deleteRelationshipUsingDELETE(videoId, id);
} catch (e) {
    print("Exception when calling MediaVideosApi->deleteRelationshipUsingDELETE: $e\n");
}
```
...
try { 
    api_instance.updateRelationshipUsingPUT(videoId, relationshipId, details);
} catch (e) {
    print("Exception when calling MediaVideosApi->updateRelationshipUsingPUT: $e\n");
}
```
