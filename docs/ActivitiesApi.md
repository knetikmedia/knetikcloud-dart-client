...
# **getActivities**
> PageResource«BareActivityResource» getActivities(filterTemplate, size, page, order)
...
try { 
    var result = api_instance.getActivities(filterTemplate, size, page, order);
    print(result);
} catch (e) {
...
try { 
    api_instance.updateActivity(id, activityResource);
} catch (e) {
