...
# **getBRETriggers**
> PageResource«BreTriggerResource» getBRETriggers(filterSystem, filterCategory, filterName, size, page)
...
try { 
    var result = api_instance.getBRETriggers(filterSystem, filterCategory, filterName, size, page);
    print(result);
} catch (e) {
...
try { 
    api_instance.updateBRETrigger(eventName, breTriggerResource);
} catch (e) {
