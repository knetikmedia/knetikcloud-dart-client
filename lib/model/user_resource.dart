...
/* The login username for the user (private). May be set to match email if system does not require usernames separately. */
  ...
  @override
  String toString()  {
    return 'UserResource[additionalProperties=$additionalProperties, address=$address, address2=$address2, avatarUrl=$avatarUrl, city=$city, countryCode=$countryCode, currencyCode=$currencyCode, dateOfBirth=$dateOfBirth, description=$description, displayName=$displayName, email=$email, firstName=$firstName, fullname=$fullname, gender=$gender, id=$id, languageCode=$languageCode, lastName=$lastName, mobileNumber=$mobileNumber, password=$password, postalCode=$postalCode, state=$state, template=$template, timezoneCode=$timezoneCode, username=$username, ]';
  }
