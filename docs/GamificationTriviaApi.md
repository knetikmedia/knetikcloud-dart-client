...
# **getQuestionTags1**
> Collection«string» getQuestionTags1(filterSearch, filterCategory, filterImportId)
...
try { 
    var result = api_instance.getQuestionTags1(filterSearch, filterCategory, filterImportId);
    print(result);
} catch (e) {
    print("Exception when calling GamificationTriviaApi->getQuestionTags1: $e\n");
}
```
...
# **getQuestions**
> PageResource«QuestionResource» getQuestions(size, page, order, filterSearch, filterIdset, filterCategory, filterTagset, filterType, filterPublished, filterImportId)
...
try { 
    var result = api_instance.getQuestions(size, page, order, filterSearch, filterIdset, filterCategory, filterTagset, filterType, filterPublished, filterImportId);
    print(result);
} catch (e) {
...
try { 
    api_instance.updateQuestionTemplate(id, questionTemplateResource);
} catch (e) {
