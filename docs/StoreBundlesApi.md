...
try { 
    var result = api_instance.createBundleItem(bundleItem);
    print(result);
} catch (e) {
...
try { 
    api_instance.updateBundleItem(id, bundleItem);
} catch (e) {
...
try { 
    api_instance.updateBundleTemplate(id, bundleTemplateResource);
} catch (e) {
