...
/* An abstract max value that the values of item's shipping_tier work against to decide whether an order can be fulfilled */
  @Property(name: 'max_tier_total')
  int maxTierTotal = null;
...
/* Whether tax should be applied to the shipping price.  Default = false */
  ...
  @override
  String toString()  {
    return 'ShippingItem[additionalProperties=$additionalProperties, behaviors=$behaviors, category=$category, createdDate=$createdDate, id=$id, longDescription=$longDescription, name=$name, shortDescription=$shortDescription, sort=$sort, tags=$tags, template=$template, typeHint=$typeHint, uniqueKey=$uniqueKey, updatedDate=$updatedDate, displayable=$displayable, geoCountryList=$geoCountryList, geoPolicyType=$geoPolicyType, shippingTier=$shippingTier, skus=$skus, storeEnd=$storeEnd, storeStart=$storeStart, vendorId=$vendorId, countries=$countries, maxTierTotal=$maxTierTotal, taxable=$taxable, ]';
  }
