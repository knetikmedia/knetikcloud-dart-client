...
/* The login username for the user (private). May be set to match email if system does not require usernames separately. */
  ...
  @override
  String toString()  {
    return 'UserResource[additionalProperties=$additionalProperties, address=$address, address2=$address2, avatarUrl=$avatarUrl, children=$children, city=$city, countryCode=$countryCode, currencyCode=$currencyCode, dateOfBirth=$dateOfBirth, description=$description, displayName=$displayName, email=$email, firstName=$firstName, fullname=$fullname, gender=$gender, id=$id, languageCode=$languageCode, lastName=$lastName, mobileNumber=$mobileNumber, parents=$parents, password=$password, postalCode=$postalCode, state=$state, tags=$tags, template=$template, timezoneCode=$timezoneCode, username=$username, ]';
  }
