...
try { 
    api_instance.updateLevel(name, newLevel);
} catch (e) {
...
try { 
    api_instance.updateUserLevel(userId, name, progress);
} catch (e) {
    print("Exception when calling GamificationLevelingApi->updateUserLevel: $e\n");
}
```
