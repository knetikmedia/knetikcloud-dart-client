...
class User {
  ...
  List<UserRelationship> children = [];
  ...
  @override
  String toString()  {
    return 'User[additionalProperties=$additionalProperties, address=$address, address2=$address2, affiliate=$affiliate, avatarUrl=$avatarUrl, children=$children, city=$city, country=$country, currency=$currency, dateCreated=$dateCreated, dateOfBirth=$dateOfBirth, dateUpdated=$dateUpdated, description=$description, displayName=$displayName, email=$email, firstName=$firstName, fullname=$fullname, gender=$gender, groups=$groups, guest=$guest, id=$id, inviteToken=$inviteToken, lang=$lang, lastActivity=$lastActivity, lastLogin=$lastLogin, lastName=$lastName, lockoutAttempts=$lockoutAttempts, lockoutDate=$lockoutDate, mobileNumber=$mobileNumber, oldId=$oldId, parents=$parents, password=$password, postalCode=$postalCode, propertiesString=$propertiesString, roles=$roles, state=$state, status=$status, tagStrings=$tagStrings, tags=$tags, template=$template, timezone=$timezone, token=$token, typeHint=$typeHint, username=$username, ]';
  }
