...
try { 
    api_instance.updateGroupTemplate(id, groupTemplateResource);
} catch (e) {
...
# **updateGroups**
> PageResource«GroupResource» updateGroups(size, page, order)
...
try { 
    var result = api_instance.updateGroups(size, page, order);
    print(result);
} catch (e) {
