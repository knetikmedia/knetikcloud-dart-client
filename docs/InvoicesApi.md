...
> PageResource«InvoiceResource» getInvoiceHistoryUsingGET(filterUser, filterEmail, filterFulfillmentStatus, filterPaymentStatus, filterItemName, filterCreatedDate, size, page, order)
...
try { 
    var result = api_instance.getInvoiceUsingGET(id);
    print(result);
} catch (e) {
...
# **getLogsUsingGET**
> PageResource«InvoiceLogEntry» getLogsUsingGET(id, size, page)
...
try { 
    api_instance.setOrderNotesUsingPUT(id, orderNotes);
} catch (e) {
    print("Exception when calling InvoicesApi->setOrderNotesUsingPUT: $e\n");
}
```
