...
try { 
    var result = api_instance.createCouponItem(couponItem);
    print(result);
} catch (e) {
...
try { 
    api_instance.updateCouponItem(id, couponItem);
} catch (e) {
...
try { 
    api_instance.updateCouponTemplate(id, couponTemplateResource);
} catch (e) {
